...
Texture2D<float4> InputScreenColor : register(t3);
...
float4 PSComposite(float4 pos : SV_Position, float2 tex : TEXCOORD0) : SV_Target
{
    ...
    color = AddVolumetricFog(color, worldPos.rgb, CameraNearFarPlanes.x, CameraNearFarPlanes.y, ViewProj, SamplerLinear, VolumetricFogVoxelGridTexture);
