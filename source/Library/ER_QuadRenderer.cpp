...
namespace Library {
	...
	ER_QuadRenderer::ER_QuadRenderer(ER_Core& game) : ER_CoreComponent(game)
	{
		...
		mIndexBuffer->CreateGPUBufferResource(rhi, indices, 6, sizeof(unsigned long), false, ER_BIND_INDEX_BUFFER, 0, 0, ER_FORMAT_R32_UINT);
		...
		inputLayoutDesc[0].InputSlotClass = true;
		...
		inputLayoutDesc[1].InputSlotClass = true;
		...
		mVS->CompileShader(rhi, ER_Utility::GetFilePath(L"content\\shaders\\Quad.hlsl"), "VSMain", ER_VERTEX, mInputLayout);
	}
