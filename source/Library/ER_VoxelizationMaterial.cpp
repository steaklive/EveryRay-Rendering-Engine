...
namespace Library
{
	...
	void ER_VoxelizationMaterial::PrepareForRendering(ER_MaterialSystems neededSystems, Rendering::RenderingObject* aObj, int meshIndex, float voxelScale, float voxelTexSize, const XMFLOAT4& voxelCameraPos)
	{
		...
		mConstantBuffer.Data.ShadowCascadeDistances = XMFLOAT4{ camera->GetCameraFarCascadeDistance(0), camera->GetCameraFarCascadeDistance(1), camera->GetCameraFarCascadeDistance(2), 1.0f };
	...
	void ER_VoxelizationMaterial::CreateVertexBuffer(Mesh& mesh, ID3D11Buffer** vertexBuffer)
	{
